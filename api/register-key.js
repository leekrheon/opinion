...
export default async function handler(req, res) {
  ...
  // YouTube API 키 형식 사전 검증 (AIzaSy + 33자 Base64url)
  if (!/^AIzaSy[A-Za-z0-9_-]{33}$/.test(key)) {
    return res.status(400).json({ error: '유효하지 않은 API KEY 형식입니다.' });
  }
  ...
  try {
    ...
    // 에러 로그에서 API 키가 노출되지 않도록 URL 내 key 파라미터를 마스킹
    const safeMsg = (e.message || '').replace(/key=[^&\s]+/gi, 'key=REDACTED');
    console.error('[/api/register-key] error:', safeMsg);
